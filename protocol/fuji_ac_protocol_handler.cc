...
namespace fuji_iot
{
    ...
    FujiAcProtocolHandler::FujiAcProtocolHandler(std::unique_ptr<FujiAcState> state)
    {
        ac_state_ = std::move(state);
    }

    absl::optional<FujiControllerFrame> FujiAcProtocolHandler::HandleMasterFrame(const FujiMasterFrame &master_frame)
    {
        // It seems that some of the frames sent over the wire are not intended for wired controller
        ...
        // There are several types of frames, they represent separate registers that hold diffrent type of information.
        switch (master_frame.Type())
        {
        case RegisterType::STATUS:
            // Status register is most commonly used (it is repeated over and over if no special actions are taken).
            // We want to make sure that local ac_state_ represents current state of the main unit, but if user
            // made changes to it, local state takes precedence and we ignore incoming data.
            if (ac_state_->Merged())
                UpdateFromMasterStatusRegister(master_frame);
            break;
        case RegisterType::LOGIN:
            // At the very beggining of the communication, wired-controller "registers" itself with the main unit.
            // This frame is sent only once upon successful registration.
        ...
        if (error_read_)
        {
            // If status frame error bit is set, error_read will be true and we will send
            // query for error register. During the next cycle, we should receive
    ...
    FujiControllerFrame FujiAcProtocolHandler::SendStatusFrame()
    {
        ...
        // wired-controller temperature sensor (I think it's in Celsius multiplied by 2).
        // AC unit ignores this value unless it's configured to use external sensor.
    ...
    void FujiAcProtocolHandler::UpdateFromMasterStatusRegister(const FujiMasterFrame &master_frame)
    {
        std::array<uint8_t, 5> payload = master_frame.Payload();
        FujiStatusRegister status(payload.data());
        if (ac_state_->ErrorFlag() != status.Error())
        {
            // If error bit is set, set the flag and query for Error register next cycle.
            error_read_ = true;
        }
        ac_state_->MergeFromMasterStatusRegister(status);
        if (!ac_state_->ControllerPresent())
        {
            // If controller is not registered with main unit, set flag and query for Login register next cycle.
            login_read_ = true;
        }
    }

    void FujiAcProtocolHandler::UpdateFromMasterErrorRegister(const FujiMasterFrame &master_frame)
    {
